	...
	class repas {
		...
		function save(){
			...
			if($this->id === false){
				$sql = "INSERT INTO repas_table VALUES (DEFAULT, "
						...
						."'".pg_escape_string($this->enchambre)."', "
						."'".pg_escape_string($this->ensalle)."', "
				...
				$sql = "UPDATE repas_table SET "
							...
							."enchambre='".pg_escape_string($this->enchambre)."', "
							."ensalle='".pg_escape_string($this->ensalle)."', "
		...
		private function readRow($row = false){
			...
			if(trim($row['id']) !== ''){
				...
				$repas->enchambre = trim($row['enchambre']);
				$repas->ensalle = trim($row['ensalle']);
